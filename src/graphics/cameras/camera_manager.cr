  ...
  module Graphics
    module Cameras
      ...
      class CameraManager
        ...
        # Active effects
        getter active_effects : Array(Effects::Effect) = [] of Effects::Effect
        ...
        getter transition_easing : Effects::Easing = Effects::Easing::EaseInOut
        ...
        def apply_effect(type : Symbol, **params)
          effect_type = case type
                        when :shake    then Effects::Type::Shake
                        when :zoom     then Effects::Type::Zoom
                        when :pan      then Effects::Type::Pan
                        when :follow   then Effects::Type::Follow
                        when :sway     then Effects::Type::Sway
                        when :rotation then Effects::Type::Rotation
                        else
          ...
          effect = case type
                   when :shake
                     ...
                     Effects::Shake.new(intensity, frequency, duration)
                   when :zoom
                     target = effect_params["target"]?.as?(Float32) || effect_params["factor"]?.as?(Float32) || 1.0f32
                     Effects::Zoom.new(target, duration)
                   when :sway
                     ...
                     Effects::Sway.new(amplitude, frequency, vertical_factor, rotation_amplitude, duration)
                   when :rotation
                     target = effect_params["target"]?.as?(Float32) || 0.0f32
                     Effects::Rotation.new(target, duration)
                   when :pan
                     ...
                     Effects::Pan.new(target_x, target_y, duration)
                   when :follow
                     ...
                     Effects::Follow.new(target, smooth, deadzone, speed, duration)
                   else
                     Effects::Effect.new(effect_type, duration, effect_params)
                   end
        ...
        private def apply_effect(effect : Effects::Effect)
          case effect.type
          when .shake?
        ...
        private def apply_shake_effect(effect : Effects::Effect)
          # If it's a ShakeEffect instance, use its method
          if effect.is_a?(Effects::Shake)
        ...
        private def apply_zoom_effect(effect : Effects::Effect)
          if effect.is_a?(Effects::Zoom)
            ...
            # Interpolate zoom
            t = effect.progress
            eased_t = apply_easing(t, Effects::Easing::EaseInOut)

            @effect_zoom = 1.0f32 + (target - 1.0f32) * eased_t
          end
        end

        private def apply_pan_effect(effect : Effects::Effect)
          if effect.is_a?(Effects::Pan)
        ...
        private def apply_follow_effect(effect : Effects::Effect)
          if effect.is_a?(Effects::Follow)
        ...
        private def apply_sway_effect(effect : Effects::Effect)
          if effect.is_a?(Effects::Sway)
        ...
        private def apply_rotation_effect(effect : Effects::Effect)
          if effect.is_a?(Effects::Rotation)
        ...
        private def apply_easing(t : Float32, easing : Effects::Easing) : Float32
          case easing
          when .linear?
            t
          when .ease_in?
