  ...
  module Core
    module Validators
      class SceneValidator
        def self.validate_scene_file(path : String) : Array(String)
          ...
          begin
            ...
            if areas = scene_data["walkable_areas"]?
              errors.concat(validate_walkable_areas(areas.as_a))
            end
        ...
        private def self.validate_walkable_areas(areas : Array(YAML::Any)) : Array(String)
          errors = [] of String
          
          areas.each_with_index do |area, index|
            prefix = "Walkable area ##{index + 1}"
            
            # Validate points
            if points = area["points"]?
              points_array = points.as_a
              if points_array.size < 3
                errors << "#{prefix}: Must have at least 3 points"
              end
              
              points_array.each_with_index do |point, pidx|
                if point["x"]? && point["y"]?
                  if point["x"].as_f < 0 || point["y"].as_f < 0
                    errors << "#{prefix}: Point ##{pidx + 1} has negative coordinates"
                  end
                else
                  errors << "#{prefix}: Point ##{pidx + 1} missing x or y coordinate"
                end
              ...
              errors << "#{prefix}: Missing required field 'points'"
            end
          end
